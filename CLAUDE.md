...
(deploy step off), zips it as `ChallengeCheat-X.Y.zip` (the `.asi` plus the default `ChallengeCheat.ini`), publishes a GitHub release named
"ChallengeCheat X.Y.Z" with that changelog entry as its notes and the zip and `.asi`
...
`bin\<Configuration>\ChallengeCheat.ini` is generated on every Debug/Release build
